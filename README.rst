...
1. **`load_data`:** This stage downloads the dataset from the URL specified in the configuration file, unzips it, and saves it as a CSV file.

2. **`clean_data`:** This stage performs the following preprocessing steps on the raw data:
...
3. **`train_model`:** This stage trains the specified model on the preprocessed data. The project supports the following models:
   - `logistic_regression`
   - `lstm`
   - `cnn`
   - `cnn_lstm`
...
4. **`evaluate_model`:** This stage evaluates the trained model on the test set and saves the evaluation metrics to a JSON file. The metrics calculated are:
   - Accuracy
   - Precision
   - Recall
   - F1-score
